<p align="left">
 <img src="https://raw.githubusercontent.com/leraniode/.github/main/assets/images/xbrandimage.png" alt="X Leraniode Logo" width="300" style="border-radius: 15px;" />
</p>

# X

Experimental and Development Packages for Leraniode/ projects

![part of leraniode](https://raw.githubusercontent.com/leraniode/.github/main/assets/badges/partofleraniode.svg)
![experimental-leraniode](https://raw.githubusercontent.com/leraniode/.github/main/assets/badges/experimentalleraniode.svg)


---

## x-py
> Experimental and development Python packages for Leraniode Projects

![python](https://img.shields.io/badge/python-3.10%2B-blue)
![license](https://img.shields.io/badge/license-MIT-green)
![Status](https://img.shields.io/badge/status-experimental-orange)

**x-py** contains the official experimental, cutting-edge Python libraries, modules, and tools developed under Leraniode.

These packages are:
- Lightweight and modular
- Focused on rapid experimentation
- Designed for prototyping new ideas, workflows, and systems

⚡ Projects in **x-py** may be unstable, change rapidly, or be replaced entirely as ideas mature.

When a project becomes stable and battle-tested, it may graduate from **X** into a standard Leraniode project.

---

## Ecosystem

All projects under **x-py** are:
- Part of the **Experimental Leraniode** ecosystem
- Maintained or curated by Leraniode
- Intended for developers who enjoy exploring new ideas early

---

## Projects

- [**`Choco`**](/choco/): A little flavoured events and action module

---

## Your Thoughts Matter

> [!NOTE]
> Leraniode thrives on community ideas and experimentation.
> So your contribution is welcomed!

If you have suggestions, feedback, or ideas for new experimental packages or improvements, join the discussion:

- 💬 [Leraniode Discussions](https://github.com/leraniode/x-py/discussions)

---

<p align="left">
x-py • Experimental Leraniode • Part of Leraniode
</p>

<img
  align="left"
  src="https://raw.githubusercontent.com/leraniode/.github/main/assets/footer/leraniodeproductbrandimage.png"
  alt="Leraniode"
  width="400"
  style="border-radius: 15px;"
/>